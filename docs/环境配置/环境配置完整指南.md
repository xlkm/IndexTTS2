# IndexTTS2 Mac M2 环境配置完整指南

**最后更新**: 2026-01-22  
**系统**: macOS (Darwin 23.5.0) - Mac Air M2

---

## 📋 目录

1. [系统环境信息](#系统环境信息)
2. [环境配置步骤](#环境配置步骤)
3. [依赖安装](#依赖安装)
4. [环境问题跟踪](#环境问题跟踪)
5. [配置差异与优化](#配置差异与优化)
6. [常见问题](#常见问题)

---

## 🖥️ 系统环境信息

### 硬件信息
- **CPU**: Apple M2 (Apple Silicon)
- **内存**: 16 GB
- **磁盘**: 460GB总容量，36GB可用（92%已使用）
- **GPU**: Apple M2 集成GPU（支持MPS加速）

### 操作系统
- **系统**: macOS
- **版本**: 14.5 (Darwin 23.5.0)
- **架构**: arm64 (Apple Silicon)
- **构建版本**: 23F79

### Python 环境
- **Python版本**: 3.11.3 ✅
- **Python路径**: `/Library/Frameworks/Python.framework/Versions/3.11/bin/python3`
- **pip版本**: 24.1 ✅
- **conda版本**: 24.3.0 ✅

### 已安装的关键库（当前状态）
- **PyTorch**: 2.0.0 ✅
- **TorchAudio**: 2.0.1 ✅
- **TorchVision**: 0.15.1 ✅
- **NumPy**: 1.26.4 ✅（已修复兼容性）
- **SciPy**: 1.10.1 ✅
- **MPS支持**: ✅ 可用（已验证）
- **Librosa**: 0.11.0 ✅
- **SoundFile**: 0.13.1 ✅
- **Pandas**: 3.0.0 ✅
- **TensorBoard**: 2.20.0 ✅
- **OmegaConf**: 2.3.0 ✅
- **Hydra-Core**: 1.3.2 ✅

---

## 🔧 环境配置步骤

### 方法1: 使用 Conda（推荐 Mac M2）

```bash
# 1. 创建 conda 环境（推荐 Python 3.10）
conda create -n indextts2 python=3.10 -y
conda activate indextts2

# 2. 安装 PyTorch（支持 MPS）
conda install pytorch torchvision torchaudio -c pytorch

# 3. 安装系统依赖
brew install libsndfile

# 4. 安装项目依赖
pip install -r requirements.txt
```

### 方法2: 使用 venv（当前 Python 3.11）

```bash
# 1. 创建虚拟环境
python3 -m venv venv
source venv/bin/activate

# 2. 安装 PyTorch（支持 MPS）
pip install torch torchvision torchaudio

# 3. 安装系统依赖
brew install libsndfile

# 4. 安装项目依赖
pip install -r requirements.txt
```

### 方法3: 使用 setup.sh 脚本

```bash
# 自动检测系统并安装
./setup.sh
```

---

## 📦 依赖安装

### 系统依赖（macOS）

```bash
# 安装音频处理所需的系统库
brew install libsndfile

# 可选：安装 ffmpeg（用于更多音频格式支持）
brew install ffmpeg
```

### Python 核心依赖

#### 已安装的核心依赖
- ✅ **libsndfile** (系统依赖)
- ✅ **soundfile** 0.13.1 - 音频I/O
- ✅ **librosa** 0.11.0 - 音频处理
- ✅ **pandas** 3.0.0 - 数据处理
- ✅ **tensorboard** 2.20.0 - 训练监控
- ✅ **omegaconf** 2.3.0 - 配置管理
- ✅ **hydra-core** 1.3.2 - 配置管理
- ✅ **numpy** 1.26.4 - 数值计算（已修复兼容性）
- ✅ **scipy** 1.10.1 - 科学计算
- ✅ **matplotlib** 3.7.1 - 可视化

#### 可选依赖（中文处理）
- ⏳ **jieba** - 中文分词（网络问题暂缓）
- ⏳ **pypinyin** - 中文拼音（网络问题暂缓）

### 验证安装

```bash
# 验证 PyTorch 和 MPS
python3 -c "import torch; print('PyTorch:', torch.__version__); print('MPS可用:', torch.backends.mps.is_available())"

# 验证音频库
python3 -c "import librosa; import soundfile as sf; print('Librosa:', librosa.__version__); print('SoundFile:', sf.__version__)"

# 验证所有依赖
python3 -c "import torch, librosa, soundfile, pandas, tensorboard, omegaconf; print('所有核心依赖已安装')"
```

---

## 📊 环境问题跟踪

### ✅ 已解决的问题

#### 1. 缺失的依赖库 ✅
**完成时间**: 2026-01-22

**解决方案**:
- ✅ 安装系统依赖：`brew install libsndfile`
- ✅ 安装音频处理库：soundfile 0.13.1, librosa 0.11.0
- ✅ 安装数据处理库：pandas 3.0.0
- ✅ 安装训练监控：tensorboard 2.20.0
- ✅ 安装配置管理：omegaconf 2.3.0, hydra-core 1.3.2
- ✅ 修复 numpy 版本兼容性（降级到 1.26.4）
- ✅ 验证音频处理功能正常

**当前状态**: 核心依赖已全部安装，可以运行 IndexTTS2 的基本功能

### ⏳ 待解决的问题

#### 2. Python 版本兼容性 ⏳
**问题**: 当前使用 Python 3.11.3，文档推荐 Python 3.10  
**优先级**: 中  
**建议**: 如果遇到兼容性问题，创建 Python 3.10 环境

#### 3. environment.yml 配置 ⏳
**状态**: 已更新（移除 CUDA，改为 Python 3.10，PyTorch 2.0+）  
**待测试**: 使用新配置创建环境并验证

#### 4. Dockerfile 配置 ⏳
**优先级**: 低  
**说明**: Mac M2 通常不需要 Docker，直接运行即可

---

## 🔍 配置差异与优化

### Python 版本建议

| 文档位置 | 推荐的Python版本 | 说明 |
|---------|-----------------|------|
| IndexTTS2 官方 | 3.7-3.10 | 通用要求 |
| Mac M2 推荐 | **3.10** | **最佳兼容性** |
| 当前系统 | 3.11.3 | ⚠️ 超出推荐范围 |

**建议**: 
- ✅ **推荐使用 Python 3.10**（Mac M2 最佳兼容性）
- ⚠️ 如果继续使用 3.11.3，需要测试兼容性

### PyTorch 版本

| 配置位置 | PyTorch版本要求 | 说明 |
|---------|---------------|------|
| requirements.txt | >=1.10.0 | 最低要求 |
| Mac M2 推荐 | **>=2.0.0** | **更好的MPS支持** |
| 当前安装 | **2.0.0** | ✅ 符合要求 |

**建议**: ✅ 当前版本 2.0.0 合适，无需更改

### environment.yml 配置

**已更新配置**（Mac M2 优化）:
```yaml
name: indextts2
channels:
  - pytorch
  - conda-forge
  - defaults
dependencies:
  - python=3.10  # ✅ Mac M2推荐版本
  - pip
  - pytorch>=2.0.0  # ✅ 更好的MPS支持
  - torchaudio>=2.0.0
  # 注意：Mac M2 不支持 CUDA，已移除 cudatoolkit
  - numpy>=1.21.0
  - scipy>=1.7.0
  - matplotlib>=3.4.0
  - pandas>=1.3.0
  - scikit-learn>=1.0.0
  - pip:
    - librosa>=0.9.0
    - soundfile>=0.10.0
    - tensorboard>=2.7.0
    - tqdm>=4.62.0
    - pyyaml>=5.4.0
    - omegaconf>=2.1.0
    - hydra-core>=1.1.0
```

---

## 🎯 Mac M2 特殊配置

### MPS (Metal Performance Shaders) 加速

```python
import torch

def get_device():
    """获取可用设备"""
    if torch.backends.mps.is_available():
        return torch.device("mps")
    elif torch.cuda.is_available():
        return torch.device("cuda")
    else:
        return torch.device("cpu")

device = get_device()
print(f"使用设备: {device}")
```

### 验证 MPS 支持

```bash
python3 -c "import torch; print('MPS可用:', torch.backends.mps.is_available()); print('MPS已构建:', torch.backends.mps.is_built())"
```

### 性能对比

| 设备 | 推理速度 | 训练速度 | 备注 |
|------|---------|---------|------|
| MPS (M2) | 中等 | 慢 | 适合推理 |
| CPU (M2) | 慢 | 很慢 | 备用方案 |
| CUDA (NVIDIA) | 快 | 快 | 最佳性能 |

---

## 🐛 常见问题

### 问题1: MPS 不支持某些操作

**解决方案**: 
- 检查 PyTorch 版本（建议 2.0+）
- 某些操作需要回退到 CPU
- 等待 PyTorch 更新支持

```python
try:
    output = model(input.to(device))
except RuntimeError as e:
    if "MPS" in str(e):
        print("MPS 不支持此操作，使用 CPU")
        output = model(input.cpu()).to(device)
```

### 问题2: 音频库安装失败

**解决方案**:
```bash
# 安装系统依赖
brew install libsndfile ffmpeg

# 使用 conda 安装
conda install -c conda-forge librosa soundfile
```

### 问题3: NumPy 版本冲突

**解决方案**:
```bash
# 如果遇到 NumPy 2.x 兼容性问题，降级到 1.x
pip install "numpy<1.27.0"
```

### 问题4: Python 版本兼容性

**解决方案**:
```bash
# 创建 Python 3.10 环境
conda create -n indextts2 python=3.10 -y
conda activate indextts2
```

---

## 📝 环境快照命令

### 导出已安装的包

```bash
# pip环境
pip freeze > requirements_installed.txt

# conda环境
conda env export > environment_snapshot.yml
```

### 检查环境

```bash
# Python信息
python3 --version
python3 -c "import sys; print(sys.executable)"

# PyTorch和MPS
python3 -c "import torch; print('PyTorch:', torch.__version__); print('MPS:', torch.backends.mps.is_available())"

# 音频库
python3 -c "import librosa; print('Librosa:', librosa.__version__)"
```

---

## 🔗 相关资源

- [PyTorch MPS 文档](https://pytorch.org/docs/stable/notes/mps.html)
- [Apple Metal Performance Shaders](https://developer.apple.com/metal/)
- [项目 README](../README.md)
- [快速开始指南](../快速开始.md)

---

## 💡 优化建议

1. **使用 MPS 进行推理**: 比 CPU 快很多
2. **批处理**: 一次处理多个样本
3. **模型量化**: 减小模型大小，提高速度
4. **云端训练**: 如需训练，考虑使用云端 GPU

---

**提示**: Mac M2 的 MPS 加速在推理时表现良好，但训练可能较慢。建议在 Mac 上进行推理，训练使用云端 GPU。

**最后更新**: 2026-01-22
